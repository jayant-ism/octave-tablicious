...
function [ numarr, txtarr, rawarr, lim, extout ] = odsread (filename, wsh=1, datrange=[], varargin)
  ...
  ods = octave.table.internal.io.odsopen (filename, 0, reqintf);
  
  if (~isempty (ods))

    [rawarr, ods, rstatus] = octave.table.internal.io.ods2oct (ods, wsh, datrange, opts);

    if (rstatus)
      [numarr, txtarr, lim] = octave.table.internal.io.parsecell (rawarr, ods.limits);
      if (! isempty (hndl) && ! isempty (numarr))
        try
          [numarr, extout] = feval (hndl, numarr);
        catch
          warning ("odsread: applying specified function handle failed with:\
error\n'%s'\n", lasterr);
        end_try_catch
      endif
    else
      warning (sprintf ("odsread: No data read from %s.\n", filename));
      rawarr = {}; numarr = []; txtarr = {}; extout = [];
    endif
  
    ods = octave.table.internal.io.odsclose (ods);
