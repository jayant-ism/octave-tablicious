...
function [ ods ] = odsopen (filename, rw=0, reqinterface=[])
  ...
  if (nargout < 1)
    error ("odsopen: no return argument specified!\n usage: ODS = odsopen (ODSfile, [Rw]).\n");
  endif

  if (! (islogical (rw) || isnumeric (rw)))
      error ("odsopen: numerical or logical value expected for arg ## 2 (readwrite)\n")
  endif

  if (ischar (filename))
    [pth, fnam, ext] = fileparts (filename);
    if (isempty (fnam))
      error ("odsopen: no filename or empty filename specified");
    endif
    if (rw && ! isempty (pth))
      apth = make_absolute_filename (pth);
      if (exist (apth) != 7)
        error ("odsopen: cannot write into non-existent directory:\n'%s'\n", ...
               apth);
      endif
    endif
  else
    error ("odsopen: filename expected for argument #1");
  endif

  if (! isempty (reqinterface))
    intfmsg = "requested";
    if (! (ischar (reqinterface) || iscell (reqinterface)))
      error ("odsopen: arg # 3 (interface) not recognized\n");
    endif
    ...
    if (isempty (regexpi (reqinterface, lastintf, "once"){1}))
      ...
      for ii=1:numel (reqinterface)
        ...
        elseif (strcmpi (reqintf, "OCT"))
          ...
          error (sprintf (["odsopen: unknown .ods interface \"%s\" requested.\n" ...
  ...
  if (fid < 0)
    if (! rw)                 ## Read mode requested but file doesn't exist
      err_str = sprintf ("odsopen: file %s not found\n", filename);
      error (err_str)
    else        
...
##      printf ("Creating file %s\n", filename);
        ...
        error (sprintf ("odsopen: write mode requested but file %s is not writable\n",...
                        filename))
      endif
    endif
  else
  ...
  if (odsinterfaces.UNO && ! odssupport && ismember (ftype, [1:3, [6:7]]))
    [ ods, odssupport, lastintf ] = ...
  ...
  if (! odssupport)
    ...
    printf ("None.\n");
    warning ("odsopen: no %s spreadsheet I/O support with %s interface(s).\n", ...
             ext, intfmsg);
    ods = [];
    chkintf = [];
  else
