...
function [ rawarr, ods, rstatus ] = ods2oct (ods, wsh=1, datrange=[], spsh_opts=[])
  ...
  if (! isstruct (ods))
    error ("ods2oct: file ptr struct expected for arg @ 1"); 
  endif
  ...
  if (test1)
    error ("ods2oct: arg #1 is an invalid ods file struct\n");
  endif
  ## Check worksheet ptr
  if (! (ischar (wsh) || isnumeric (wsh)))
    error ("ods2oct: integer (index) or text (wsh name) expected for arg # 2\n");
  elseif (isempty (wsh))
    wsh = 1;
  endif
  ## Check range
  if (! (isempty (datrange) || ischar (datrange)))
    error ("ods2oct: character string (range) expected for arg # 3\n");
  ...
  elseif (! isstruct (spsh_opts))
    error ("ods2oct: struct expected for OPTIONS argument (# 4)\n");
  else
  ...
  if (strcmp (ods.xtype, "OTK"))
    ...
    switch ods.odfvsn
      ...
      otherwise
        error ("ods2oct: unsupported odfdom version or invalid ods file pointer.\n");
    endswitch
  elseif (strcmp (ods.xtype, "JOD"))
    ...
    if ((ods.odfvsn < 4) && spsh_opts.formulas_as_text)
      ...
      warning (["ods2oct: option 'formulas_as_text' not supported\n", ...
  ...
  if (spsh_opts.convert_utf)
    if (exist ("native2unicode", "file"))
      ...
      conv_fcn = @octave.table.internal.io.utf82unicode;
    endif
    rawarr = octave.table.internal.io.tidyxml (rawarr, conv_fcn);
  endif
