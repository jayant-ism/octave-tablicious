## Copyright (C) 2009-2019 by Philip Nienhuis
##
...
function [ numarr, txtarr, rawarr, lims, extout ] = xlsread (fn, wsh, datrange, varargin)
  ...
  if (nargin < 1) 
    ...
    return
  elseif (! ischar (fn))
    error ("xlsread: filename (text string) expected for argument #1, not a %s\n", class (fn));
  elseif (nargin == 1)
    wsh = 1;
    datrange = ""; 
  elseif (nargin == 2)
  ...
  if strcmpi (ext, ".csv")
    ...
    unwind_protect  ## Needed to catch COM errors & able to close stray Excel
      ...
      xls_ok = 0;
      xls = octave.table.internal.io.xlsopen (fn, 0, reqintf);
      if (! isempty (xls))
        xls_ok = 1;
 
        ## Get data from spreadsheet file & return handle
        [rawarr, xls, rstatus] = octave.table.internal.io.xls2oct (xls, wsh, datrange, opts);
