...
function [ rstatus ] = odswrite (filename, data, wsh=1, crange="", reqintf=[])
  ...
  if (nargin < 2)
    error ("odswrite: insufficient arguments - see 'help odswrite'\n");
  endif

  ods = octave.table.internal.io.odsopen (filename, 1, reqintf);

  if (! isempty (ods)) 
    [ods, rstatus] = octave.table.internal.io.oct2ods (data, ods, wsh, crange);
