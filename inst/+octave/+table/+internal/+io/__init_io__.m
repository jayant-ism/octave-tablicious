...
## PKG_ADD: octave.table.internal.io.__init_io__ ()
  ...
  #   ## Who knows where OSX keeps e.g., Apache POI stuff? if it does at all...
    ...
    try
      if (! isempty (userdir))
        ...
        if (exist (userdir) == 7)
          ## Userdir is a subdir
          spr_status = octave.table.internal.io.chk_spreadsheet_support (userdir, 0, ooopath);
        endif
      ...
      ## Try <HOME>/java
      spr_status = octave.table.internal.io.chk_spreadsheet_support ([ homedir filesep "java" ], 0, ooopath);
      ## Only then search for system-supplied jars. ooopath has been searched
      spr_status = octave.table.internal.io.chk_spreadsheet_support ([ libdir filesep "java" ], 0);
    catch
      warning ("io: (automatic loading of spreadsheet I/O Java classlibs failed)\n");
    end_try_catch
  endif
