...
function [ rstatus ] = xlswrite (filename, arr, arg3, arg4, arg5)
  ...
  ## Sanity checks
  if (nargin < 2)
    error ("xlwrite: insufficient arguments - see 'help xlswrite'\n");
  ...
  if ((nr > nrows) || (nc > ncols))
    ...
    warning ("xlswrite: array truncated to %d by %d to fit in range %s\n", ...
             nrows, ncols, crange);
    ## Adapt crange
    crange = [ octave.table.internal.io.calccelladdress(trow, lcol) ":" ...
               octave.table.internal.io.calccelladdress(trow+nrows-1, lcol+ncols-1) ];
  endif

  unwind_protect          ## Needed to be sure Excel can be closed i.c.o. errors
    xls_ok = 0;
    xls = octave.table.internal.io.xlsopen (filename, 1, reqintf);
    if (! isempty (xls))
      xls_ok = 1;

      [xls, rstatus] = octave.table.internal.io.oct2xls (arr(1:nr, 1:nc), xls, wsh, crange);
    endif

  unwind_protect_cleanup
    if (xls_ok && ! isempty (xls))
      xls = octave.table.internal.io.xlsclose (xls);
    endif
